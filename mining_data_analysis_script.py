...
if pure_df.shape[0] == 0:
    ...
    if idle_stat1.shape[0] == 0:
        # here divide current dataset into 2 regarding status_0 in order to properly calculate idling time for locations
        # only with status0_5. If this condition works, the dataframe consists only of idling time with status 0 = 5
        ...
        def dict_idle(df):
            ...
            while k <= len(df):
                ...
                if df.loc[i + 1, 'y_coor'] - df.loc[i, 'y_coor'] <= 5:
                    while df.loc[i + 1, 'y_coor'] - df.loc[i, 'y_coor'] <= 5:
        ...
        check_stat5 = coor_blocks(dict_stat5_raw, 'status0_1')
        ...
        def total_idle_plot(df):
            colors = ['red' if x == 0 else 'green' for x in df['status0_5']]
        ...
        def idling_report(df):
            with open("{}_idling_result.txt".format(vehicle_short), "a") as f:
                ...
                if (df['status0_5'] == 0).any():
                    print('Total idling time with status "loaded/standing" -', df.query('status0_1==0')['idling'].sum(),
                          file=f)
        ...
        final_joined_df, stat_5 = coor_split(final_joined_df)
        ...
        def stops_map_plot(df5):
            ...
            return plt.savefig('{}_idling_locations.png'.format(vehicle_short), facecolor='white', bbox_inches='tight')


        dump_truck = vehicle_raw
        date = date_raw
        stops_map_plot(stat_5)
